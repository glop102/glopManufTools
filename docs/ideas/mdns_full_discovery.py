...
def scan(service: str, iface: str, timeout: int = 5) -> None:
    ...
    services: dict[
        str, MDNSServiceData
    ] = {}  # instance_name -> MDNSServiceData (for SRV/TXT lookup)
    ...
    with _make_socket(iface) as sock:
        ...
        while (remaining := deadline - time.monotonic()) > 0:
            ...
            for rr in _iter_rrs(dns):
                ...
                response_time = (
                    (now - query_times[rrname]) if rrname in query_times else None
                )
                ...
                if rtype == TYPE_PTR:
                    ...
                    if svc is not None:
                        ...
                        if target not in hosts:
                            hosts[target] = MDNSHostData(
                                interface=iface, hostname=target
                            )
                        host = hosts[target]
                        ...
                        for rec in response_records:
                            if (
                                rec.rtype in (TYPE_A, TYPE_AAAA)
                                and rec.rrname == target
                                and rec.rdata not in host.addresses
                            ):
    ...
    print(
        f"  {'time':>8}  {'rt_ms':>7}  {'src_ip':<40}  {'type':<5}  {'ttl':>6}  {'rrname':<45}  rdata"
    )
    for rec in response_records:
        rt = (
            f"{rec.response_time * 1000:7.1f}"
        ...
        print(
            f"  {ts}  {rt}  {rec.src_ip:<40}  {rec.type_name:<5}[{shared}]  {rec.ttl:>6}  {rec.rrname:<45}  {rec.rdata}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Base listen timeout in seconds (default: 5)",
    )
